...
def forecast(
    ...
    with torch.no_grad():
        u_pred = odeint(model._ode_func, u0, t_forecast, method="dopri5")

    # Drop the first point (= last training observation)
    t_out = t_forecast[1:]
    u_out = u_pred[1:]
...
def forecast_differences(
    ...
    with torch.no_grad():
        for i in range(steps):
            t_cur = torch.tensor(t_last + dt * i, dtype=torch.float64)
            u = model._known_map(u, p, t_cur) + model._network_module(u)
            states.append(u.numpy())
            times.append(float(t_cur.item()) + dt)
